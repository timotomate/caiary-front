...
class ReviewUpdatePage extends StatefulWidget {
  ...
  @override
  _ReviewUpdatePageState createState() => _ReviewUpdatePageState();
}
...
class _ReviewUpdatePageState extends State<ReviewUpdatePage> {
  ...
  void initState() {
    ...
    super.initState();
    myArticle = widget.article;
    controller.text = myArticle.content;
    location_controller.text = myArticle.location;
    menu_controller.text = myArticle.menu;
    music_controller.text = myArticle.song;

    feeling_good = feeling_soso = feeling_bad = false;
    myArticle.emotion == "good" ? feeling_good = true : widget.article.emotion == "soso" ? feeling_soso = true : feeling_bad = true;

    menu = myArticle.menu.isEmpty ? false : true;
    weather = myArticle.weather.isEmpty ? false : true;
    music = myArticle.song.isEmpty ? false : true;
    rating = myArticle.point == 0 ? false : true;

    sunny = cloudy = windy = rain = snow = false;
    if(myArticle.weather.isNotEmpty) {
      myArticle.weather == "Sunny" ? sunny = true : myArticle.weather == "Cloud" ? cloudy = true : myArticle.weather == "Windy" ? windy = true : myArticle.weather == "Rain" ? rain = true : snow = true;
    }

    userRating = myArticle.point == 0 ? 3.0 : myArticle.point.toDouble();
    _imageUrl = myArticle.image;
  }
  ...
  Future<void> postRequest() async {
    ...
    Map<String, dynamic> map = {
      "emotion" : "${myArticle.emotion}",
      "location" : "${myArticle.location}",
      "menu" : "${myArticle.menu}",
      "weather" : "${myArticle.weather}",
      "song" : "${myArticle.song}",
      "point" : "${myArticle.point}",
      "content" : "${myArticle.content}",
    };//심문성

    if (_image != null) {
      map['image'] = await MultipartFile.fromFile(_image.path);
    }
    ...
    try{
      ...
      var response = await dio.put(
          'https://caiary-server.herokuapp.com/articles/update/${widget.article.id}/',
          data: formData,
          options: Options(
                ...
                children: [
                  ...
                  Column(
                    children: [
                      ...
                      SizedBox(height: 5),
                      RatingBar.builder(
                        initialRating: myArticle.point.toDouble(),
                        minRating: 1,
